...
from src.otimizando_sistema_bancario import (
    depositar, sacar, encontrar_conta, exibir_extrato,
    criar_usuario, filtrar_usuario, criar_conta, listar_contas, menu
)
    ...
    return [
        {
            "numero_conta": "12345",
            "agencia": "0001",
            "usuario": {"cpf": "111.222.333-44", "nome": "João", "endereco": "Rua A, 123"},
            "saldo": 500.0,
            "extrato": "",
            "numero_saques": 0,
            "limite": 500,
        ...
        },
        {
            "numero_conta": "67890",
            "agencia": "0001",
            "usuario": {"cpf": "555.666.777-88", "nome": "Maria", "endereco": "Rua B, 456"},
            "saldo": 1000.0,
            "extrato": "",
            "numero_saques": 0,
            "limite": 500,
...
def test_depositar_conta_nao_encontrada(contas_fixture, monkeypatch, capsys):
    ...
    contas_atualizadas = depositar(contas_fixture, cpf="999.888.777-66", numero_conta="00000")
    captured = capsys.readouterr()
    assert "Operação falhou! Conta não encontrada para o CPF informado.\n" in captured.out
    assert contas_atualizadas == contas_fixture
...
def test_sacar_limite_excedido(contas_fixture, monkeypatch, capsys):
    ...
    # Configurar conta com limite menor
...
def test_sacar_limite_saques_excedido(contas_fixture, monkeypatch, capsys):
    ...
    # Configurar conta com número máximo de saques
    conta = encontrar_conta(contas_fixture, "111.222.333-44", "12345")
...
def test_exibir_extrato_valido(contas_fixture, monkeypatch, capsys):
    # Primeiro faz um depósito para ter extrato
    monkeypatch.setattr('builtins.input', lambda _: "200")
    depositar(contas_fixture, cpf="111.222.333-44", numero_conta="12345")

    # Agora exibe o extrato
...
def test_criar_usuario_valido(usuarios_fixture, monkeypatch, capsys):
    inputs = ["123.456.789-00", "Carlos Silva", "20/10/1980", "Rua C, 789 - Centro - Belo Horizonte/MG"]
    input_iter = iter(inputs)
    monkeypatch.setattr('builtins.input', lambda _: next(input_iter))
...
def test_criar_conta_usuario_nao_encontrado(usuarios_fixture, monkeypatch, capsys):
    ...
    captured = capsys.readouterr()
    assert "Usuário não encontrado, fluxo de criação de conta encerrado." in captured.out
    assert conta is None


# Testes para listar contas
...
if __name__ == "__main__":
    pytest.main([__file__, "-v", "--cov=src.otimizando_sistema_bancario", "--cov-report=term-missing"])